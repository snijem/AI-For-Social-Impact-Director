...
async function connectDB() {
  try {
    ...
    // Log config without exposing password (security)
    ...
    // Debug password parsing (only in development)
    if (process.env.NODE_ENV === 'development' && dbConfig.password) {
      const pwd = dbConfig.password
      console.error('Password debug - Length:', pwd.length)
      console.error('Password debug - First 3 chars:', JSON.stringify(pwd.substring(0, 3)))
      console.error('Password debug - Last 3 chars:', JSON.stringify(pwd.substring(Math.max(0, pwd.length - 3))))
      console.error('Password debug - Raw env value length:', process.env.DB_PASSWORD ? process.env.DB_PASSWORD.length : 0)
    }
