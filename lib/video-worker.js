...
export async function processGenerationJob(jobId) {
  ...
  try {
    ...
    const scenesData = {
      ...
      totalDuration: DEFAULT_DURATION_SECONDS,
      provider: 'runway',
      model: 'runway-ml'
    }
    ...
    await queryDB(
      ...
      [
        'completed',
        finalProgress,
        `Completed: ${DEFAULT_DURATION_SECONDS}-second video generated with Runway`,
        JSON.stringify(scenesData),
        jobId
      ]
    )
    
    console.log(`[Video Worker] Job ${jobId} completed: ${DEFAULT_DURATION_SECONDS}-second video generated: ${mergedVideoUrl}`)
