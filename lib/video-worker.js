...
const DURATION = '9s'
const MAX_CLIPS = 1 // Only 1 clip for 9s generation
const TARGET_SECONDS = 9 // 9 seconds for single clip
const COST_PER_CLIP = 0.25 // $0.25 per clip
...
export async function processGenerationJob(jobId) {
  ...
  try {
    ...
    // Limit to MAX_CLIPS (1 clip for 9s generation)
    ...
    // Log all video URLs to debug
    console.log(`[Video Worker] Collected ${videoUrls.length} video URLs for merging:`)
    videoUrls.forEach((url, index) => {
      console.log(`  [${index + 1}] ${url}`)
    })
    
    // Remove duplicates (in case continuation returns same video)
    const uniqueVideoUrls = [...new Set(videoUrls)]
    if (uniqueVideoUrls.length !== videoUrls.length) {
      console.warn(`[Video Worker] Found ${videoUrls.length - uniqueVideoUrls.length} duplicate video URLs, removing duplicates`)
      console.log(`[Video Worker] Using ${uniqueVideoUrls.length} unique videos for merging`)
    }
    ...
    console.log(`[Video Worker] Merging ${uniqueVideoUrls.length} clips into one video...`)
    ...
    try {
      if (videoUrls.length > 1) {
        ...
        if (ffmpegAvailable) {
          try {
            // Merge videos (use unique URLs)
            await doMerge(uniqueVideoUrls, mergedVideoPath)
            
            // Generate public URL via API route (more reliable than direct public folder access)
            ...
            mergedVideoUrl = uniqueVideoUrls[0]
          }
        } else {
