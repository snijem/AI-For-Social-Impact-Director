...
export async function setupDatabase() {
  ...
  try {
    ...
    // Create lives_log table to track all lives changes
    await queryDB(`
      CREATE TABLE IF NOT EXISTS lives_log (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        previous_lives INT NOT NULL,
        new_lives INT NOT NULL,
        change_amount INT NOT NULL,
        action_type ENUM('decrement', 'admin_set', 'admin_reset', 'initial', 'refund') NOT NULL,
        reason VARCHAR(255) NULL,
        admin_user_id INT NULL,
        related_job_id VARCHAR(255) NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        INDEX idx_user_id (user_id),
        INDEX idx_created_at (created_at),
    ...
    `)
    console.log('✓ Lives log table created/verified')

    // Migrate video_url column if it exists as VARCHAR(500)
    try {
      await queryDB(`
        ALTER TABLE user_videos 
        MODIFY COLUMN video_url TEXT NULL
      `)
      console.log('✓ Migrated video_url column to TEXT')
    } catch (migError) {
      // Column might not exist or already be TEXT, ignore
      if (!migError.message.includes('Duplicate column name') && !migError.message.includes('Unknown column')) {
        console.log('Note: Could not migrate video_url column:', migError.message)
      }
    }
    ...
    // Migrate result_url column if it exists as VARCHAR(500)
    try {
      await queryDB(`
        ALTER TABLE result_links 
        MODIFY COLUMN result_url TEXT NOT NULL
      `)
      console.log('✓ Migrated result_url column to TEXT')
    } catch (migError) {
      // Column might not exist or already be TEXT, ignore
      if (!migError.message.includes('Duplicate column name') && !migError.message.includes('Unknown column')) {
        console.log('Note: Could not migrate result_url column:', migError.message)
      }
    }
